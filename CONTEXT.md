...
preprocess-scripts/         # 官方维护的预处理脚本源码（构建入口在 scripts/build-preprocess-scripts.mjs 中写死）
  community-registry.json   # 社区脚本注册表：手动维护，贡献者以 URL 提交（vault-relative/https/ipfs/internal.ipfs-locked）
...
- **社区注册表（community registry）**：`preprocess-scripts/community-registry.json`，手动维护。贡献社区脚本 = 提交一个 URL 条目（`name`/`description`/`scriptURL`），脚本本身不搬进仓库、不参与构建、不被锁定。
- **生成索引（generated index）**：`src/preprocess/script-index.generated.json`，由 `scripts/update-preprocess-index.mjs` 生成（正式脚本条目来自构建配置元数据 + 产物 CID，合并社区注册表条目），提交进 repo、由插件运行时 import 供设置下拉使用。`.generated` 后缀明确表示这是构建产物而非手改源文件。
