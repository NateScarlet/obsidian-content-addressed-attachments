<script module lang="ts">
	...
	const { t } = defineLocales({
		en: {
			...
			lockCancelledByUser: "Lock cancelled by user",
		},
		zh: {
			lockProgress: "文件迁移进度",
			...
			close: "关闭",
			lockFailed: "❌ 迁移失败",
			lockCancelled: "⏹️ 迁移已取消",
			cancel: "取消",
			cancelling: "取消中...",
			remainingFiles: (remaining: number) => `剩余 ${remaining} 个文件`,
			lockCancelledByUser: "迁移已被用户取消",
		},
	});
	//#endregion
</script>
...
<div class="p-5 max-w-[600px] lock-progress-modal">
	<h2 class="text-xl font-semibold mb-4 mt-0 text-normal">
		{t("lockProgress")}
	</h2>

	<div class="font-semibold mb-3 text-normal lock-progress-text">
		{progressText}
	</div>
	...
	{#if progress.status === "completed" && hasMeaningfulDetails}
		<details class="mt-3 lock-details">
			<summary
